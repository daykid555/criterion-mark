// frontend/src/pages/PrintingHistoryPage.jsx
import React, { useState, useEffect } from 'react';
import apiClient from '../api';
import { Link } from 'react-router-dom';

const STATUS_STYLES = { PRINTING_COMPLETE: 'bg-green-200 text-green-800', IN_TRANSIT: 'bg-cyan-200 text-cyan-800', DELIVERED: 'bg-gray-200 text-gray-800' };
const HistoryTable = ({ jobs }) => {
    if (jobs.length === 0) return <p className="text-center py-10 text-white/70">No completed jobs found.</p>;
    return (
        <div className="overflow-x-auto">
            <table className="w-full text-left text-white min-w-[900px]">
                <thead className="border-b border-white/20 text-sm text-white/70">
                    <tr><th className="p-4">Batch ID</th><th className="p-4">Product</th><th className="p-4">Manufacturer</th><th className="p-4">Quantity</th><th className="p-4">Status</th><th className="p-4 text-center">Downloads</th></tr>
                </thead>
...
function PrintingHistoryPage() {
  ...
  useEffect(() => {
    const fetchData = async () => {
        ...
        try {
            const historyRes = await apiClient.get('/api/printing/history');
  ...
  return (
    <div className="p-4 sm:p-6 lg:p-8">
      <h1 className="text-3xl sm:text-4xl font-bold text-white mb-8 drop-shadow-lg">Completed History</h1>
