...
// --- VerificationResult Component (No Changes Needed Here) ---
const VerificationResult = ({ result }) => {
  const { status, message, data, firstScanDetails } = result;
  ...
  const isDuplicate = status === 'error' && result.message.includes('ALREADY BEEN VERIFIED');
  
  let cardClass = 'bg-green-50/80 border-green-500';
  let titleClass = 'text-green-800';
  let title = 'Product Verified!';

  if (isDuplicate) {
    cardClass = 'bg-yellow-50/80 border-yellow-500';
    titleClass = 'text-yellow-800';
    title = 'Warning: Duplicate Scan ⚠️';
  }

  return (
    <div className={`p-6 border-l-4 ${cardClass} rounded-lg shadow-md mt-6`}>
      <h3 className={`text-2xl font-bold ${titleClass}`}>{title}</h3>
      <p className="mt-1 text-gray-800">{message}</p>
      
      {isDuplicate && firstScanDetails && (
        <div className="mt-3 pt-3 border-t border-yellow-400/50 text-sm text-yellow-900">
          <p><strong>First verification was on:</strong> {new Date(firstScanDetails.scannedAt).toLocaleString()}</p>
          <p><strong>Location of first scan:</strong> {firstScanDetails.location}</p>
        </div>
      )}
