...
// ... (Logo and HamburgerIcon components remain the same)
const Logo = () => ( <Link to="/" className="flex flex-col items-center -space-y-1"> <span className="text-xs text-white/80 tracking-widest">THE</span> <span className="text-xl font-bold text-white tracking-wider">CRITERION</span> <span className="text-sm text-white/80 tracking-widest">MARK</span> </Link> );
const HamburgerIcon = ({ open }) => ( <div className="flex flex-col justify-around w-6 h-6"> <span className={`h-0.5 w-full bg-white transition-transform duration-300 ${open ? 'rotate-45 translate-y-[0.5rem]' : ''}`}></span> <span className={`h-0.5 w-full bg-white transition-opacity duration-300 ${open ? 'opacity-0' : ''}`}></span> <span className={`h-0.5 w-full bg-white transition-transform duration-300 ${open ? '-rotate-45 -translate-y-[0.5rem]' : ''}`}></span> </div> );
...
function Navbar() {
  ...
  // THIS IS THE FIX: Get auth state and logout function from context
        ...
        <div className="glass-panel max-w-7xl mx-auto">
            ...
            <div className="flex items-center justify-between h-16 px-4 sm:px-6 lg:px-8">
                ...
                <div className="hidden md:flex items-center space-x-4">
                    ...
                    {/* THIS IS THE FIX: Conditional Login/Logout Button */}
                    {isAuthenticated ? (
                ...
                {/* Hamburger Button */}
                <div className="md:hidden z-20"><button onClick={() => setIsOpen(!isOpen)} className="p-2"><HamburgerIcon open={isOpen} /></button></div>
            </div>
            ...
            <div className={`md:hidden absolute top-0 left-0 w-full h-screen pt-24 px-8 bg-gray-900/80 backdrop-blur-xl flex flex-col items-center space-y-6 text-2xl transition-transform duration-300 ease-in-out ${isOpen ? 'transform-none' : '-translate-x-full'}`}>
                ...
                <Link to="/verify" onClick={() => setIsOpen(false)} className="text-white/80 hover:text-white font-medium">Verify Product</Link>
                 {/* THIS IS THE FIX: Conditional Login/Logout Button for Mobile */}
                {isAuthenticated ? (
